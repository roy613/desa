<style>
    @page {
        sheet-size: A4-L;
    }
</style>

<?php $this->load->view('kop_surat'); ?>
<html>
<font face="arial">

    <table border=0 style="margin-top:15px; width:100%; font-size:12pt">
        <tr>
            <td style="text-align:center">
                <h4><b><u>SURAT KETERANGAN KEMATIAN</u></b></h4>
            </td>
        </tr>
        <tr>
            <td style="text-align:center">
                <p>Nomor : gfsghfdhyfcjhgcvujkg</p>
            </td>
        </tr>
        <tr>
            <td>
                <br>

            </td>
        </tr>
    </table>
    <p style="margin-left: 15px; font-size:12pt"> Yang bertandatangan di bawah ini menerangkan bahwa :</p>
    <table border=0 style="margin-top:10px; margin-left:20px; margin-bottom:10px; width:100%; font-size:12pt">
        <tr>
            <td style="width:33%">
                Nama Lengkap
            </td>
            <td style="width: 3%;">
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
        <tr>
            <td>
                Tempat Tanggal Lahir
            </td>
            <td>
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
        <tr>
            <td>
                Warga Negara
            </td>
            <td>
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
        <tr>
            <td>
                Agama
            </td>
            <td>
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
        <tr>
            <td>
                Pekerjaan
            </td>
            <td>
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
        <tr>
            <td>
                Tempat Tinggal Terakhir
            </td>
            <td>
                :
            </td>
            <td>
                qqqqqqqqqqqqqqqqqqqqqqq
            </td>
        </tr>
    </table>
    <table border=0 style="margin-top:0px; margin-left:15px; width:100%; font-size:12pt; padding:2px">
        <tr>
            <td style="text-align: justify;">
                Adalah benar warga Desa Karangan Hilir Kecamatan Karangan Kabupaten Kutai Timur dan telah
                Meninggal Dunia pada hari xxxx Tanggal xxx Pukul xxxx WITA bertempat di xxxxxxxx di Jl. xxxxxx RT. xxxxx Desa Karangan Hilir Kecamatan Karangan Kabupaten Kutai Timur.
            </td>
        </tr>
        <tr>
            <td>
                
            </td>
        </tr>
        <tr>
            <td style="text-align: justify;">
                Demikian Surat Keterangan ini diberikan agar dapat dipergunakan sebagaimana mestinya.
            </td>
        </tr>
    </table>
    <table border=0 style="width:100%; font-size:12pt;margin-top:30px; margin-left:400px">
        <tr>
            <td><br></td>
        </tr>
        <tr>
            <td style="width:40%">
                Dibuat di
            </td>
            <td style="width:4%">
                :
            </td>
            <td>
                Karangan Hilir
            </td>
        </tr>
        <tr>
            <td>
                Pada Tanggal
            </td>
            <td>
                :
            </td>
            <td>
                30 September 2022
            </td>
        </tr>
        <tr>
            <td colspan="3">
                <hr style="margin-top:-1px; height:1px; width:100%; color:black; text-align:left">
            </td>
        </tr>
    </table>
    <table border=0 style="width: 100%;  font-size:12pt">
        <tr>
            <td style="width:30%;"></td>
            <td style="width:30%;"></td>
            <td style=" text-align:center;"></td>
        </tr>
        <tr>
            <td rowspan="5" style="width:30%; text-align:center"><img src="#" style="height:110px"></td>
            <td style="width:30%;"></td>
            <td style=" text-align:center;"><b>Kepala Desa</td>
        </tr>
        <tr>
            <td colspan="2" style="color: white;"> <br><br><br></td>
        </tr>
        <tr>
            <td></td>
            <td style="text-align:center"> <b><u>dfgfdgfdgfdg</u></b></td>
        </tr>

        <tr>
            <td></td>
            <td style="text-align:center">dfgfdgfdg</td>
        </tr>

        <tr>
            <td></td>
            <td style="text-align:center">fdgfgfdgfgdfg</td>
        </tr>

    </table>
</font>

</html>